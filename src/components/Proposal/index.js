...
import { createMilestone, setDone } from '../../contract'
...
class Proposal extends Component {
  ...
  handleClick = async (person, id, milestone) => {
    if (person === 'client') {
      try {
        await createMilestone(milestone, true)
        console.log('Signed', 'Client:', true)
      ...
      try {
        await createMilestone(milestone, false)
        console.log('Signed', 'Client:', false)
        const proposalNew = await axios.put(
                    ...
                    <List key={index} component="ul">
                      ...
                      {milestone.client.signed ? (
                        ...
                        <Button
                          variant="contained"
                          onClick={() =>
                            this.handleClick('client', milestone.id, milestone)
                          }
                        >
